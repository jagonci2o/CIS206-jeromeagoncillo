...
def main():
    ...
    def addEmployeeToList():
        # Add current employee to the list and display in listbox
        ...
        # Show full employee info in listbox
        listbox.insert(
            tk.END,
            f"Name: {new_emp.getName()} | ID: {new_emp.getId()} | Dept: {new_emp.getDepartment()} | Pos: {new_emp.getPosition()}"
        )
    ...
    def showSelectedEmployee(event):
        # Show full info when selecting from the listbox
    ...
    # New listbox to display employee list
    listbox = tk.Listbox(root, width=80)
    listbox.pack(pady=10)
    listbox.bind('<<ListboxSelect>>', showSelectedEmployee)  # Bind click to display info
